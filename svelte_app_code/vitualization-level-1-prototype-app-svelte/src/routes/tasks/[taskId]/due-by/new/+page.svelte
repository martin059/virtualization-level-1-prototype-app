<script lang="ts">
  ...
  onMount(() => {
    newDueBy = { task_id: taskId, due_date: new Date().toISOString().split('T')[0], is_active: true };
    submitEnabled = true;
    isLoading = false;
  });
  ...
  async function handleSubmit() {
    if (submitEnabled){
      ...
      if (statusCode >= 200 && statusCode < 300) {
        acts.add({ mode: 'success', message: 'New due date created successfully. Redirecting to previous page...', lifetime: 2});
        await new Promise(resolve => setTimeout(resolve, 2000)); // Wait 2 seconds so user can read message
        goto('/tasks/' + taskId + '/due-by');
      } else if (statusCode == 409) {
        acts.add({ mode: 'error', message: 'Due date for ' + newDueBy.due_date + ' already exists for this task, please either select a different one or go back to the task\'s due by list and activate it.', lifetime: 5 });
        console.log('Due date for ' + newDueBy.due_date + ' already exists for this task, please either select a different one or go back to the task\'s due by list and activate it.');
      } else {
