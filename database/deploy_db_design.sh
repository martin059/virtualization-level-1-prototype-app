...
docker cp $SCRIPTPATH/simple-app-db-v1.0.sql vitualization-level-1-sample-app-db-1:/tmp;

command_output=$(docker exec vitualization-level-1-sample-app-db-1 psql -U postgres -a -f /tmp/simple-app-db-v1.0.sql | tr -d '\r');
