...
docker cp $SCRIPTPATH/simple-app-db-v1.0.sql virtualization-level-1-prototype-app-db-1:/tmp;

command_output=$(docker exec virtualization-level-1-prototype-app-db-1 psql -U postgres -a -f /tmp/simple-app-db-v1.0.sql | tr -d '\r');
